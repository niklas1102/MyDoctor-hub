...
def appointments_home(request):
    return HttpResponse("Welcome to the Appointments system.")
...
def appointment_request(request, service_id=None, staff_member_id=None):
    ...
    if staff_member_id:
        ...
        available_slots_raw = get_available_slots_for_staff(date.today(), staff_member)
        available_slots = [slot.strftime('%I:%M %p') for slot in available_slots_raw]

    
    staff_id_from_query = request.GET.get('staff')
    if staff_id_from_query and staff_id_from_query != 'none':
        try:
            staff_member = StaffMember.objects.get(id=staff_id_from_query)
            available_slots_raw = get_available_slots_for_staff(date.today(), staff_member)
            available_slots = [slot.strftime('%I:%M %p') for slot in available_slots_raw]
        except StaffMember.DoesNotExist:
            staff_member = None

    elif service_id and all_staff_members and all_staff_members.count() == 1:
