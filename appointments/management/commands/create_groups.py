...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f'Group "{patient_group.name}" created')
            )
        if created:
            self.stdout.write(
                self.style.SUCCESS(f'Group "{doctor_group.name}" created')
            )
        ...
        if not User.objects.filter(username="admin").exists():
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    'Superuser "admin" created and assigned to Admin group'
                )
            )
